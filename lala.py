...
class Model(tf.keras.Model):
    def __init__(self, num_actions):
        ...
        # there is a track that combining the two streams' fc layer, then
        # the output of that layer is a |A| + 1 dimension tensor
...
class MSTDAgent:  # Multi-Step TD Learning Based on Dueling Double DQN with Proportional Prioritization
    def __init__(self, model, target_model, env, learning_rate=.01, epsilon=.1, epsilon_dacay=0.995, min_epsilon=.01,
                 gamma=.9, batch_size=8, target_update_iter=400, train_nums=5000, buffer_size=300, replay_period=20,
                 alpha=0.4, beta=0.4, beta_increment_per_sample=0.001, n_step=1):
        self.model = model
        self.target_model = target_model
        # gradient clip
        opt = ko.Adagrad(learning_rate=learning_rate, clipvalue=10.0)  #, clipvalue=10.0
