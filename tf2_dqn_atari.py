...
# Minor change from cs234:reinforcement learning, assignment 2 -> preprocess.py
def greyscale(state):
    """
...
class DQNAgent:
    def __init__(self, model, target_model, env, buffer_size=1000, learning_rate=.001, epsilon=.1, gamma=.9,
                 batch_size=4, target_update_iter=20, train_nums=100, start_learning=90):
        self.model = model
    ...
    def test(self, render=True):
        ...
        while not done:
            obs = greyscale(obs)
            action, q_value = self.model.action_value(obs[None])  # Using [None] to extend its dimension [80, 80, 1] -> [1, 80, 80, 1]
